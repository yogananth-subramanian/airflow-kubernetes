...
if [[ $PLATFORM == "baremetal" ]]; then
    ...
    if [[ ${workload} == "kraken" ]]; then
        ...
        echo "Transfering the environment variables to the orchestration host"
        scp -o 'StrictHostKeyChecking=no' -o 'UserKnownHostsFile=/dev/null' -i ${PRIVATE_KEY}  /home/kni/clusterconfigs/auth/kubeconfig root@${CERBERUS_HOST}:/root/.kube/config
